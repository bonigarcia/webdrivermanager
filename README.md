...
| ``browserVersionDetectionRegex(String)``    | Regular expression used to extract the browser version from the shell. By default, the regex ``[^\\d^\\.]``                                                                                                                                                                                                                                                  | ``wdm.browsreVersionDetectionRegex``                                                                                                                                                                                 |
...
| ``commandsPropertiesUrl(URL)``              | URL of the ``commands.properties`` file, used the find out the commands to be used to detect the browser version. Local URLs (``file://`` are allowed). By default this value targets to the master branch of [commands.properties] in  GitHub.                                                                                                              | ``wdm.versionsPropertiesUrl``                                                                                                                                                                                        |
...
| ``exportParameter(String)``                 | Java property name used to export the driver path (e.g., by default, ``webdriver.chrome.driver`` for Chrome or ``webdriver.gecko.driver`` for Firefox).                                                                                                                                                                                                      | ``wdm.chromeDriverExport``, ``wdm.geckoDriverExport``, ``wdm.edgeDriverExport``, ``wdm.internetExplorerDriverExport``, ``wdm.operaDriverExport``, ``wdm.phantomjsDriverExport``                                      |
...
[INFO]
[INFO] ------------------------------------------------------------------------
[INFO] Building WebDriverManager 4.4.2
[INFO] ------------------------------------------------------------------------
[INFO]
...
```
> java -jar webdrivermanager-4.4.2-fat.jar server
[INFO] WebDriverManager server listening on port 4041
```
...
```
docker run -p 4041:4041 bonigarcia/webdrivermanager:4.4.2
```
...
```
docker run --rm -e BROWSER=chrome -e WDM_CHROMEVERSION=84 -e WDM_OS=LINUX -v ${PWD}:/wdm bonigarcia/webdrivermanager:4.4.2
```

... or Mac:

```
docker run --rm -e BROWSER=chrome -e WDM_CHROMEVERSION=84 -e WDM_OS=MAC -v ${PWD}:/wdm bonigarcia/webdrivermanager:4.4.2
```
...
```
docker run --rm -e BROWSER=chrome -e WDM_CHROMEVERSION=84 -e WDM_OS=WIN -v %cd%:/wdm bonigarcia/webdrivermanager:4.4.2
```
...
[fat-jar]: https://github.com/bonigarcia/webdrivermanager/releases/download/webdrivermanager-4.4.2/webdrivermanager-4.4.2-fat.jar
