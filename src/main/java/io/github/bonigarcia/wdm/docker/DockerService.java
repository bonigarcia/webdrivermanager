...
public class DockerService {
    ...
    private DockerClient getDockerClient(String dockerHost) {
        ...
        setAuthConfig(dockerClientConfig.getAuthConfigurations().getConfigs());
        return DockerClientBuilder.getInstance(dockerClientConfig)
                .withDockerHttpClient(dockerHttpClient).build();
    }

    private void setAuthConfig(Map<String, AuthConfig> configs) {
        String endpoint = config.getDockerPrivateEndpoint();
        String username = config.getDockerPrivateUsername();
        String password = config.getDockerPrivatePassword();
        if (StringUtils.isNotBlank(endpoint) && StringUtils.isNotBlank(username) && StringUtils.isNotBlank(password)) {
            AuthConfig authConfig = new AuthConfig()
                    .withUsername(username)
                    .withPassword(password);
            configs.put(endpoint, authConfig);
        }
    }
