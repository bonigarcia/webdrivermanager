...
// *WebDriverManager* is a library which allows to automate the management of the drivers (e.g. _chromedriver_, _geckodriver_, etc.) required by https://www.selenium.dev/[Selenium WebDriver].
...
*WebDriverManager* (Copyright © 2017-2021) is a project created by http://bonigarcia.github.io/[Boni Garcia] (https://twitter.com/boni_gg[@boni_gg]) licensed under http://www.apache.org/licenses/LICENSE-2.0[Apache 2.0 License]. This documentation is released under the terms of https://creativecommons.org/licenses/by/3.0/[CC BY 3.0] (also available in link:webdrivermanager.pdf[PDF]). There are several ways to get in touch with WebDriverManager:
