...
https://github.com/bonigarcia/webdrivermanager[WebDriverManager] is an open-source Java library that carries out this <<driver-management,driver management>> process in a fully automated manner. In addition, as of version 5, WebDriverManager provides other relevant features, such as the ability to <<browser-finder,discover browsers>> installed in the system, <<webdriver-builder,building WebDriver objects>> (such as `ChromeDriver`, `FirefoxDriver`, etc.), and the capability to run <<browsers-in-docker,browsers in Docker containers>> seamlessly.
...
* `getInstance(Class<?> webDriverClass)`: Where `webDriverClass` is a class of the Selenium WebDriver standard hierarchy, such as `ChromeDriver.class`, `FirefoxDriver.class`, etc.
...
* `getDriverVersions()`: Used to find out the list of available driver versions in a given manager.
...
* Method `remoteAddress()`: To specify the remote URL in the `RemoteWebDriver` instance (typically when using a Selenium Server). See https://github.com/bonigarcia/webdrivermanager/blob/master/src/test/java/io/github/bonigarcia/wdm/test/create/ChromeRemoteTest.java[example]. This method is equivalent to the configuration key `wdm.remoteAddress` (see <<advanced-configuration,configuration>> section).
...
==== Advanced Settings
The dockerized browser provided by WebDriverManager can be configured in different ways. For example, the WebDriverManager API allows using volumes, customizing the language and timezone inside the browser container, using custom images, configuring remote Docker daemon, customizing shared memory and in-memory filesystem (_tmpfs_), changing the screen resolution and video frame rate, customizing the recording output (folder and filename), or accessing to Docker service and client to make custom operations with Docker containers (e.g., run commands in the browser container). See the <<advanced-configuration,advanced configuration>> section for specific details about it.
...
NOTE: In addition to the methods presented in this section, each manager provides a method called `config()` that allows configuring all the possible parameters of WebDriverManager.
...
[source,shell]
----
docker run --rm -v ${PWD}:/wdm -e ARGS="resolveDriverFor=chrome" -e WDM_CHROMEVERSION=84 bonigarcia/webdrivermanager:5.0.0
----
...
|`useMirror()`|`wdm.useMirror=true`|`false` (not using mirrors)|Enable the use of a driver repository https://npm.taobao.org/mirrors/[mirror] (available for chromedriver, geckodriver, and operadriver)
...
|`avoidExport()`|`wdm.avoidExport=true`|`false` (export driver paths as Java properties (e.g. `webdriver.chrome.driver`)|Avoid step 4 in the <<resolution-algorithm,resolution algorithm>> (for instance, in the <<wdm-cli,CLI>> mode)
|`exportParameter(String)`|`wdm.chromeDriverExport`, `wdm.geckoDriverExport`, `wdm.edgeDriverExport`, `wdm.iExplorerDriverExport`, `wdm.operaDriverExport`|Java property name used to export the driver path (available in https://github.com/bonigarcia/webdrivermanager/blob/master/src/main/resources/webdrivermanager.properties[webdrivermanager.properties])|Set custom property name
|`avoidOutputTree()`|`wdm.avoidOutputTree=true`|`false` (create output tree in the driver cache, e.g., `chromedriver/linux64/2.37`|Avoid output tree (for instance, in the <<wdm-cli,CLI>> mode)
...
|`dockerTimezone(String)`|`wdm.dockerTimezone`|`Etc/UTC`|https://en.wikipedia.org/wiki/List_of_tz_database_time_zones[Timezone] in browser container
|`dockerNetwork(String)`|`wdm.dockerNetwork`|`bridge`|Docker network name
|`dockerLang(String)`|`wdm.dockerLang`|`EN`|https://en.wikipedia.org/wiki/List_of_ISO_639-1_codes[Languge] in Docker container
...
|`dockerRecordingOutput(String)` `dockerRecordingOutput(Path)`|`wdm.dockerRecordingOutput`|`.`|Path for the recording output
|`dockerRecordingPrefix(String)`|`wdm.dockerRecordingPrefix`|Browser name|Prefix to be appended to the session id (in the recording filename)
|`dockerCustomImage(String)`|`wdm.dockerCustomImage`|`""`|Custom image to be used as browser in Docker
| `dockerVolume(String)` `dockerVolumes(String[])`|`wdm.dockerVolumes`|`""`|Comma-separated volumes (or string array) using the format `"\local\path:\container\path"`
|=======
...
[discrete]
==== Chrome 92+ in Docker
There is an https://bugs.chromium.org/p/chromedriver/issues/detail?id=3857[open issue] with Chrome 92+ and Docker at the time of this writing. The problem can be solved to some extent by using the argument `--disable-gpu` in Chrome and Edge. This argument is used by WebDriverManager 5 to avoid the issue. Nevertheless, some situations are still impossible to fix (e.g., when using https://hub.docker.com/r/bonigarcia/webdrivermanager[WebDriverManager in Docker] as CLI or Server).

== Support
There are two ways to try to get community support related WebDriverManager. First, questions about WebDriverManager can be discussed in https://stackoverflow.com/questions/tagged/webdrivermanager_java[StackOverflow], using the tag _webdrivermanager_java_. In addition, comments, suggestions, and bug-reporting should be made using the https://github.com/bonigarcia/webdrivermanager/issues[GitHub issues].
...
In addition, both https://opencollective.com/webdrivermanager[WebDriverManager] and https://opencollective.com/selenium-jupiter[Selenium-Jupiter] are part of https://opencollective.com/bonigarcia[OpenCollective], an online funding platform for open and transparent communities. You can contribute with the project as a https://opencollective.com/webdrivermanager/donate[donation] (i.e., an individual custom contribution), as a https://opencollective.com/webdrivermanager/contribute/backer-8132/checkout[backer] (i.e., an individual recurring contribution),  or as https://opencollective.com/webdrivermanager/contribute/sponsor-8133/checkout[sponsor] (i.e., a recurring contribution by a company).

== Further Documentation
There are other resources related to WebDriverManager and automated testing you can find helpful. For instance, the following journal papers:
...
* García, Boni, et al. "http://ieeexplore.ieee.org/document/7992926/[WebRTC testing: challenges and practical solutions]." _IEEE Communications Standards Magazine_ 1.2 (2017): 36-42.
* García, Boni, and Juan Carlos Dueñas. "http://www.rintonpress.com/journals/jweonline.html#v14n56[Web browsing automation for applications quality control]." _Journal of web engineering_ (2015): 474-502.
