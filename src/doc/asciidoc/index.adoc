...
Some of the drivers (e.g., geckodriver or operadriver) are hosted on GitHub. When external clients (like WebDriverManager makes) many consecutive requests to GitHub, eventually, it responds with an HTTP 403 error (_forbidden_). 
...
io.github.bonigarcia.wdm.config.WebDriverManagerException: Error HTTP 403 executing https://api.github.com/repos/mozilla/geckodriver/releases
    ...
    at io.githu.bonigarcia.wdm.WebDriverManager.manage(WebDriverManager.java:877)
...
To avoid this problem, WebDriverManager can make authenticated requests using a personal access token. See the <<configuration,configuration>> section to discover how to set up this specify in WebDriverManager.
